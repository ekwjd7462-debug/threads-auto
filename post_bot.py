...
# v8 실측 재분석: TOP/조합이 전체의 절반이라 피드가 단조로워짐 → 구조 다양화.
# 세운시사형 중앙값 최상위, 조합·TOP은 대박형(고편차). 특징·일상·통찰 섞어 리듬 살림.
# "상담후기형" 신설 (하루 2회, 08·22시): 현실적 사례 서사. 주작 금지.
# 트랙 배분: 정보 17 / 후기 2 / 모객 2 / 전환 2 / 성장 1 (합 24)
DAY_PLAN = {
    0:  {"track": "정보", "engine": "조합랭킹형", "category": "인간관계",
         "note": "인복·귀인 부르는 조합 랭킹"},
    1:  {"track": "정보", "engine": "세운시사형", "category": "운세",
         "note": "병오년 하반기 일간별 흐름 (새벽1시 중앙값 1.9천 실측)"},
    2:  {"track": "정보", "engine": "통찰에세이형", "category": "인생",
         "note": "새벽 3시 감성 사유 글. 용어 없이 삶을 뒤집는 한 문장"},
    3:  {"track": "정보", "engine": "일상판별리스트형", "category": "연애",
         "note": "궁합·인연 판별 일상 체크리스트"},
    4:  {"track": "정보", "engine": "특징해부형", "category": "연애",
         "note": "오행 과다자 연애 해부 (○다자 패턴 공식)"},
    5:  {"track": "정보", "engine": "조합랭킹형", "category": "재물",
         "note": "돈 쓸어 담는·돈 냄새 맡는 조합 랭킹"},
    6:  {"track": "모객", "engine": "떡밥관찰형", "category": "직업",
         "note": "일·직장 통념 뒤집는 관찰. 댓글로 답 쏟아지게"},
    7:  {"track": "정보", "engine": "조합랭킹형", "category": "연애",
         "note": "이성 줄 서는·주도권 쥐는 조합 랭킹 (아침 강세 시간)"},
    8:  {"track": "후기", "engine": "상담후기형", "category": "재물",
         "note": "돈·직업 고민으로 상담 온 사람이 작은 결정 하나 바꾼 현실 사례"},
    9:  {"track": "정보", "engine": "TOP리스트형", "category": "재물",
         "note": "돈복·재물 그릇 리스트 (이 시간 최고 5.2만 실측)"},
    10: {"track": "정보", "engine": "특징해부형", "category": "재물",
         "note": "재성·무재 사주 특징 해부"},
    11: {"track": "정보", "engine": "일상판별리스트형", "category": "인간관계",
         "note": "귀인·악연 판별 체크리스트 (약한 시간대 보강)"},
    12: {"track": "정보", "engine": "TOP리스트형", "category": "결혼",
         "note": "점심 피크. 배우자·결혼 반전 리스트 (대박 소재)"},
    13: {"track": "모객", "engine": "생년참여형", "category": "운세",
         "note": "댓글에 생년 던지게 판 깔기"},
    14: {"track": "정보", "engine": "조합랭킹형", "category": "연애",
         "note": "연애 주도권·헤어나올 수 없는 조합 랭킹 (3만 실측 시간)"},
    15: {"track": "정보", "engine": "특징해부형", "category": "인간관계",
         "note": "특정 살·오행 인간관계 패턴 해부 (약한 시간대 보강)"},
    16: {"track": "정보", "engine": "텐션불릿형", "category": "연애",
         "note": "정착 상대 특징 불릿 (외모·성격·결정적 신호)"},
    17: {"track": "정보", "engine": "세운시사형", "category": "운세",
         "note": "병오년 하반기 일간별 운세 (최강 시간·최고 엔진 2만 실측)"},
    18: {"track": "정보", "engine": "TOP리스트형", "category": "연애",
         "note": "퇴근길. 연애 반전 리스트"},
    19: {"track": "성장", "engine": "연재예고형", "category": "신살",
         "note": "신살 시리즈 연재. 오늘의 살 하나 + 내일 예고 한 줄 (팔로우·저장 구걸 금지)"},
    20: {"track": "전환", "engine": "상담가이드형", "category": "상담",
         "note": "사주 볼 때 꼭 물어봐야 할 것들 + 마지막 한 줄 '나를 찾아오면 됨'"},
    21: {"track": "정보", "engine": "TOP리스트형", "category": "재물",
         "note": "골든타임. 돈 버는 팔자·재물운 리스트"},
    22: {"track": "후기", "engine": "상담후기형", "category": "연애",
         "note": "밤. 연애·결혼 고민으로 상담 온 사람의 담담한 변화 서사"},
    23: {"track": "전환", "engine": "시기포착형", "category": "대운",
         "note": "뭘 해도 안 풀리는 시기의 정체. 마지막 한 줄만 '나를 찾아와' 유도"},
}
...
TRACK_RULES = {
    ...
    "전환": ("이 글은 전환 트랙임. 독자가 '내 사주 한번 제대로 확인해야겠다'는 생각이 들도록 "
             ...
             "과장·공포팔이 금지. 마지막 한 줄에만 담백하게 상담 유도: '궁금하면 나를 찾아와' "
             "'제대로 알고 싶으면 나를 찾아오면 됨' '나를 찾아오시길' 결의 표현만 사용. "
             "'프로필' '링크' '타고 와' 같은 표현은 절대 금지. 그 이상 팔면 실패작."),
    "성장": ("이 글은 성장 트랙임. 연재 시리즈의 한 편처럼 쓰고 끝에 다음 편 예고 한 줄로만 닫을 것. "
             "'팔로우' '저장' 유도는 절대 쓰지 말 것. 다음 편이 궁금해지게 만드는 예고 자체가 미끼임. 상담 언급 금지."),
    "후기": ("이 글은 상담후기 트랙임. 사주 상담 받고 삶이 조금 바뀐 실제 사례처럼 쓰되 "
             "**절대 주작 티가 나면 안 됨**. 지켜야 할 현실성 규칙: "
             "(1) 인생역전·대박·기적 금지. 바뀐 건 아주 작고 구체적인 것 하나 (예: '이직을 6개월 미룸' "
             "'소개팅 나가는 요일을 바꿈' '돈 나가는 습관 하나를 끊음'). "
             "(2) 손님을 특정하지 말 것. '작년에 온 30대 후반 직장인' 정도의 흐릿한 소개만. 이름·직업 디테일 과잉 금지. "
             "(3) 안 바뀐 것도 솔직히 인정. '드라마틱하게 팔자가 뒤집힌 건 아님. 근데' 같은 한 줄. "
             "(4) 상담가 자화자찬 금지. '내가 용해서'가 아니라 '그 사람이 움직여서'. "
             "(5) 사주 근거는 딱 하나만 담백하게 (예: '재성이 약한데 비겁이 세서 돈이 나갈 자리였음'). "
             "(6) 극적인 대사·눈물·반전 연출 금지. 미담 큰따옴표 남발 금지. "
             "(7) 마지막은 담백하게. 상담 강매 금지. 유도하려면 '나를 찾아오시길' 정도 한 번만, 안 넣어도 됨. "
             "읽는 사람이 '어 이거 좀 진짜 같네, 내 상황이랑 비슷한데' 하고 조용히 설득되면 성공."),
}
...
- **말투 고정 검사: 반말 음슴체만 허용.** 존댓말체(~습니다 ~해요 ~하세요 ~이에요 ~십시오)가 있으면
  전부 반말 음슴체(~임 ~함 ~됨 ~더라)로 바꿀 것. "죄송하지만 ~됩니다"도 "미안한데 ~됨"으로.
- **금지 문구 제거: "저장" 유도(저장해둬/저장각), "팔로우" 구걸, "프로필/링크/타고 와"가 있으면 삭제.**
  상담 유도가 필요하면 오직 "나를 찾아와" "나한테 와" "나를 찾아오면 됨" "나를 찾아오시길"로만 교체.
