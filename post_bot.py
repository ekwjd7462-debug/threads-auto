...
# v10 콘텐츠 전면 개편: 명언·인생의 진리·공감·위로형 감성 계정으로 전환.
# 단문(명언형·단문선언형)을 넉넉히 섞어 여백의 미를 살림.
# 트랙 배분: 감성 22 / 사주위로 2 (하루 두 번만 사주 렌즈의 위로)
DAY_PLAN = {
    0:  {"track": "감성", "engine": "명언형", "category": "인생",
         "note": "자정. 하루를 닫는 짧은 명언 한 줄"},
    1:  {"track": "감성", "engine": "위로형", "category": "위로",
         "note": "새벽. 잠 못 드는 사람에게 건네는 담담한 위로"},
    2:  {"track": "감성", "engine": "단문선언형", "category": "인생",
         "note": "새벽 2시. 한 방으로 남는 두 줄"},
    3:  {"track": "감성", "engine": "인생진리형", "category": "인생",
         "note": "새벽 3시. 살면서 알게 되는 진리 하나"},
    4:  {"track": "감성", "engine": "위로형", "category": "관계",
         "note": "관계에 지친 사람에게 건네는 위로"},
    5:  {"track": "감성", "engine": "명언형", "category": "인생",
         "note": "동트기 전. 짧은 명언"},
    6:  {"track": "감성", "engine": "공감형", "category": "일상",
         "note": "아침. 출근하는 사람 마음 짚는 공감"},
    7:  {"track": "감성", "engine": "단문선언형", "category": "인생",
         "note": "출근길. 한 줄 선언"},
    8:  {"track": "사주위로", "engine": "사주위로형", "category": "운",
         "note": "아침. 안 풀리는 시기를 사주 렌즈로 다독임. 끝에만 '나를 찾아와' 한 번"},
    9:  {"track": "감성", "engine": "명언형", "category": "인생",
         "note": "오전. 짧은 명언"},
    10: {"track": "감성", "engine": "공감형", "category": "관계",
         "note": "오전. 사람 관계 공감"},
    11: {"track": "감성", "engine": "인생진리형", "category": "돈",
         "note": "돈·일에 관한 담담한 진리"},
    12: {"track": "감성", "engine": "위로형", "category": "위로",
         "note": "점심. 오늘 하루 버티는 사람에게 위로"},
    13: {"track": "감성", "engine": "단문선언형", "category": "인생",
         "note": "오후. 두 줄 선언"},
    14: {"track": "감성", "engine": "명언형", "category": "관계",
         "note": "오후. 관계에 관한 짧은 명언"},
    15: {"track": "감성", "engine": "공감형", "category": "일상",
         "note": "나른한 오후. 지침·번아웃 공감"},
    16: {"track": "감성", "engine": "인생진리형", "category": "인생",
         "note": "오후. 인생의 진리 하나"},
    17: {"track": "감성", "engine": "위로형", "category": "위로",
         "note": "퇴근 무렵. 애쓴 하루를 다독임"},
    18: {"track": "감성", "engine": "명언형", "category": "인생",
         "note": "퇴근길. 짧은 명언"},
    19: {"track": "감성", "engine": "공감형", "category": "관계",
         "note": "저녁. 관계·외로움 공감"},
    20: {"track": "사주위로", "engine": "사주위로형", "category": "운",
         "note": "저녁. 힘든 시기를 사주로 위로. 끝에만 '나를 찾아오면 됨' 한 번"},
    21: {"track": "감성", "engine": "위로형", "category": "위로",
         "note": "밤. 하루 끝 위로"},
    22: {"track": "감성", "engine": "인생진리형", "category": "인생",
         "note": "밤. 곱씹게 되는 진리"},
    23: {"track": "감성", "engine": "단문선언형", "category": "인생",
         "note": "자정 직전. 한 줄로 닫기"},
}

ENGINE_PARTS = {
    "명언형": (1, 1), "단문선언형": (1, 1), "인생진리형": (1, 2),
    "공감형": (1, 2), "위로형": (1, 2), "사주위로형": (1, 2),
}

TRACK_RULES = {
    "감성": ("이 글은 감성 트랙임. 명언·인생의 진리·공감·위로 중 하나로, 읽는 사람 마음에 조용히 남을 것. "
             "지식 자랑·정보 나열·사주 용어 금지. 훈계·설교 금지. 담담하게, 여백을 남기고. "
             "상담·프로필·팔로우·저장 언급 절대 금지. 그냥 좋은 글 하나를 건넨다는 마음으로 쓸 것."),
    "사주위로": ("이 글은 사주위로 트랙임. 사주를 아주 가벼운 렌즈로만 써서 힘든 사람을 위로할 것 "
                "(예: '안 풀리는 시기는 게으른 게 아니라 대운이 바뀌는 중임'). "
                "사주 용어 나열·풀이 금지. 공포·불안 조장 금지. 위로가 먼저, 사주는 거들 뿐. "
                "마지막 한 줄에만 담백하게: '힘들면 나를 찾아와' '혼자 버겁거든 나를 찾아오면 됨' 정도. "
                "'프로필' '링크' '팔로우' '저장' 표현 금지. 그 이상 팔면 실패작."),
}
...
- 엔진({engine})의 형식과 길이는 유지. 단문형(명언형·단문선언형)은 절대 늘리지 말고 짧게 둘 것.
...
## 검사 항목
1. 내부 모순: 앞 문장과 뒤 문장이 충돌하지 않는가.
2. 진부함: 너무 뻔한 클리셰("아프니까 청춘이다" 류)면 한 끗 다르게 비틀 것. 다만 억지 반전은 금지.
3. 설교·훈계: 독자를 가르치려 드는 명령조가 과하면 담담한 관찰·고백으로 완화.
4. 공감의 진정성: 위로·공감 글이 가볍거나 입에 발린 소리로 들리면, 구체적인 장면 하나로 진짜처럼 만들 것.
   단 특정인을 지어낸 사연으로 만들지는 말 것.
5. 맞춤법·띄어쓰기: 오탈자와 띄어쓰기 오류 전부 교정.
6. 최근 글과의 충돌·중복: 아래 최근 글과 주제·문장이 겹치면 각도를 바꿀 것.
7. 사주위로 트랙일 때만: 사주 언급이 명리적으로 틀리지 않았는가 (안 풀리는 시기=대운 교체기 정도의
   가벼운 비유는 OK). 공포·불안 조장이 있으면 위로로 바꿀 것. 사주 용어 나열은 빼고 위로를 살릴 것.
{avoid_block}
